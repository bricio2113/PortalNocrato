...
const CalendarView: React.FC<CalendarViewProps> = ({ empresaId }) => {
    ...
    return (
        <div className="text-zinc-100 font-sans selection:bg-[#FABE01] selection:text-black">
            ...
            {/* Header: Flex col no mobile para não quebrar */}
            <header className="mb-6 md:mb-8 flex flex-col md:flex-row md:items-end justify-between gap-4">
                ...
                <button
                    ...
                    className="w-full md:w-auto bg-[#FABE01] hover:bg-[#FABE01]/90 text-black font-bold py-3 md:py-2.5 px-6 rounded-sm shadow-[0_0_20px_rgba(250,190,1,0.2)] transition-all flex items-center justify-center gap-2 text-sm uppercase tracking-wide"
                >
            ...
            <div className="bg-[#111111] rounded-sm border border-white/5 shadow-2xl overflow-hidden">
                ...
                {isLoading ? (
                    ...
                    <div className="overflow-x-auto">
                        <div className="grid grid-cols-7 bg-[#1A1A1A] min-w-[700px]"> {/* min-w garante scroll horizontal se necessário */}
                                    ...
                                    <div key={date.toISOString()} className={`group relative min-h-[140px] p-2 border-b border-r border-white/5 flex flex-col transition-colors ${isTodayDate ? 'bg-[#FABE01]/5' : 'bg-[#111111] hover:bg-[#1A1A1A]'}`}>
                                        <div className="flex justify-between items-start mb-2">
                                            ...
                                            <button
                                                ...
                                                className="opacity-100 md:opacity-0 md:group-hover:opacity-100 p-1 text-zinc-500 hover:text-[#FABE01] hover:bg-[#FABE01]/10 rounded-sm transition-all"
                                            >
                                        ...
                                        <div className="flex-1 space-y-1.5 overflow-y-auto overflow-x-hidden custom-scrollbar">
                                            {dayEvents.map(event => (
                                                <div
                                                    ...
                                                    className={`cursor-pointer p-1.5 rounded-sm text-xs font-medium border-l-2 transition-colors ${event.type === 'POST' ? 'bg-blue-500/10 border-blue-500 text-blue-200' : 'bg-purple-500/10 border-purple-500 text-purple-200'}`}
                                                >
                                                    <div className="flex items-center gap-1.5">
                                                        {event.plataforma === 'Instagram' ? <Instagram className="w-3 h-3 opacity-70 shrink-0" /> : <FileText className="w-3 h-3 opacity-70 shrink-0" />}
